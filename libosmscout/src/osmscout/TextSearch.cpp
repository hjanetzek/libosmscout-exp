...
namespace osmscout
{
  ...
  bool TextSearch::Load(const std::string &path)
  {
    ...
    if(triesAvail==0) {
      std::cerr << "TextSearch: No valid text data files is available" << std::endl;
      return false;
    }
  ...
  bool TextSearch::Search(const std::string &query,
                          bool searchPOIs,
  ...
  void TextSearch::splitSearchResult(const std::string &result,
                                     std::string &text,
