...
void Parser::OST() {
		while (!(la->kind == _EOF || la->kind == 4 /* "OST" */)) {SynErr(49); Get();}
		Expect(4 /* "OST" */);
		if (la->kind == 6 /* "GRADES" */) {
			GRADES();
		}
...
void Parser::GRADES() {
		while (!(la->kind == _EOF || la->kind == 6 /* "GRADES" */)) {SynErr(50); Get();}
		Expect(6 /* "GRADES" */);
		GRADE();
...
void Parser::TYPES() {
		while (!(la->kind == _EOF || la->kind == 11 /* "TYPES" */)) {SynErr(51); Get();}
		Expect(11 /* "TYPES" */);
		TYPE();
...
void Parser::TAGS() {
		while (!(la->kind == _EOF || la->kind == 18 /* "TAGS" */)) {SynErr(52); Get();}
		Expect(18 /* "TAGS" */);
		TAG();
...
void Parser::GRADE() {
		...
		while (!(la->kind == _EOF || la->kind == 7 /* "SURFACE" */)) {SynErr(53); Get();}
		Expect(7 /* "SURFACE" */);
		Expect(8 /* "GRADE" */);
		UINT(grade);
		Expect(9 /* "{" */);
...
void Parser::TYPE() {
		...
		while (!(la->kind == _EOF || la->kind == 12 /* "TYPE" */)) {SynErr(54); Get();}
		Expect(12 /* "TYPE" */);
		IDENT(name);
...
void Parser::TAG() {
		while (!(la->kind == _EOF || la->kind == 19 /* "TAG" */)) {SynErr(55); Get();}
		Expect(19 /* "TAG" */);
		Expect(_string);
...
void Parser::TYPEOPTION(TypeInfo& typeInfo) {
		...
		case 36 /* "FIND_CITY" */: {
			Get();
			typeInfo.SetFindCity(true); 
			break;
		}
		case 37 /* "FIND_STREET" */: {
			Get();
			typeInfo.SetFindStreet(true); 
			break;
		}
		case 38 /* "CONSUME_CHILDREN" */: {
			Get();
			typeInfo.SetConsumeChildren(true); 
			break;
		}
		case 39 /* "OPTIMIZE_LOW_ZOOM" */: {
			Get();
...
bool Parser::StartOf(int s)
{
  ...
  const bool x = false;
	...
	static bool set[3][50] = {
		{T,x,x,x, T,x,T,T, x,x,x,T, T,x,x,x, x,x,T,T, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,T,x, x,T,T,T, T,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x},
		{x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,x,x,x, x,T,T,T, T,T,T,T, T,T,T,T, T,x,x,x, x,x}
	};
...
void Errors::SynErr(int line, int col, int n)
{
  ...
  switch (n) {
			...
			case 24: s = coco_string_create("\"IN\" expected"); break;
			...
			case 28: s = coco_string_create("\"EXISTS\" expected"); break;
			case 29: s = coco_string_create("\"NODE\" expected"); break;
			...
			case 36: s = coco_string_create("\"FIND_CITY\" expected"); break;
			case 37: s = coco_string_create("\"FIND_STREET\" expected"); break;
			case 38: s = coco_string_create("\"CONSUME_CHILDREN\" expected"); break;
			case 39: s = coco_string_create("\"OPTIMIZE_LOW_ZOOM\" expected"); break;
			case 40: s = coco_string_create("\"IGNORE\" expected"); break;
			case 41: s = coco_string_create("\"MULTIPOLYGON\" expected"); break;
			case 42: s = coco_string_create("\"PIN_WAY\" expected"); break;
			case 43: s = coco_string_create("\"IGNORESEALAND\" expected"); break;
			case 44: s = coco_string_create("\"ROUTE\" expected"); break;
			case 45: s = coco_string_create("\"FOOT\" expected"); break;
			case 46: s = coco_string_create("\"BICYCLE\" expected"); break;
			case 47: s = coco_string_create("\"CAR\" expected"); break;
			case 48: s = coco_string_create("??? expected"); break;
			case 49: s = coco_string_create("this symbol not expected in OST"); break;
			case 50: s = coco_string_create("this symbol not expected in GRADES"); break;
			case 51: s = coco_string_create("this symbol not expected in TYPES"); break;
			case 52: s = coco_string_create("this symbol not expected in TAGS"); break;
			case 53: s = coco_string_create("this symbol not expected in GRADE"); break;
			case 54: s = coco_string_create("this symbol not expected in TYPE"); break;
			case 55: s = coco_string_create("this symbol not expected in TAG"); break;
			case 56: s = coco_string_create("invalid TAGBOOLCOND"); break;
			case 57: s = coco_string_create("invalid TAGBINCOND"); break;
			case 58: s = coco_string_create("invalid TYPEKIND"); break;
			case 59: s = coco_string_create("invalid TYPEOPTION"); break;
