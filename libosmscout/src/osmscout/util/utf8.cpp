/*  $Id: utf8.cpp 45745 2010-05-12 18:16:26Z ivanov $
 * ===========================================================================
 *
...
//
char StringToChar(const std::string&      src,
                  size_t*            seq_len,
                  bool               ascii_table,
...
//
std::string Utf8Util::StringToAscii(const std::string& src, bool downcase)
{
    std::string  dst;      // String to result
    ...
    {
        // Process one UTF character
        ch = StringToChar(src.data() + i, &utf_len, true);
        // Add character to the result vector
        if ( ch != kSkipChar ){
          if (ch == 0){
            dst += '?';
          }else
            dst += downcase ? tolower(ch) : ch;
        }
...
//        kOutrangeChar or hSkipChar.
//
long StringToCode(const std::string&      src,
                  size_t*            seq_len,
                  EConversionStatus* status)
{
...
//
std::vector<long> StringToVector (const std::string& src)
{
    std::vector<long> dst;      // String to result
